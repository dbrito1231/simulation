...
- Respect the critical invariants from CLAUDE.md, especially: new actions must stay in sync across `DECISION_ACTIONS`/`DECISION_SCHEMA`/`SYSTEM_PROMPT` (server.py), `apply_decision()` + `available_actions` (sim_engine.py), and `ACTION_LABELS` (viewer.js); `simulation/roles.json` is the single source of truth for roles; the engine mutates state only under its lock.
...
- **Last step before reporting done, if you started/restarted/touched the server:** verify only one `simulation/server.py` process is running (`pgrep -fa "simulation/server.py"` or the PowerShell equivalent in CLAUDE.md) and kill any duplicates, keeping the most recently started one. Multiple instances fighting over port 5001/`state.db` has been a recurring problem.
