...
| `specs/04-http-api.md` | All 19 routes: method, request/response shape, purpose; `/state` payload key inventory; `/agent/think` marked legacy | 1,000 |
| `specs/05-world.md` | World geometry, 15 starter districts + frontier founding, road network, terrain tiles, ecology stocks/regrow/terraform, structures (registry, levels ≤100, upgrades, decay/ruins), composable blocks, cemetery | 1,300 |
...
The inventory above is a map, **not** a citable source. Every number/name/count/constant written into a doc must be line-verified in code at write time: derive the 35 actions from `DECISION_ACTIONS`, the 19 routes from `@app.route` decorators, the flag set from module-level assignments in sim_engine.py, the 12 roles from roles.json, districts from `STARTER_DISTRICTS`. Each implementer returns a fact table (claim → `file:line`) with its report; orchestrator spot-checks ≥5 entries per report. Code wins over this plan on any conflict — flag discrepancies.
...
| 1A — specs/00,01,02 | ☐ not started | | |
| 1B — specs/03,04 | ☐ not started | | |
| 2C — specs/05,06,07 | ☐ not started | | |
| 2D — specs/08,09,10 | ☐ not started | | |
