...
# Sidebar polling runs every three seconds.  Parsed records remain valid until
# either source file changes identity (path, byte size, or mtime).
_PARSE_CACHE: dict[
...
def _file_identity(path: str) -> tuple[str, int | None, int | None]:
    """Return the cache identity for a log source, including absent files."""
    try:
        stat = os.stat(path)
    except OSError:
...
def _read_audit_sources(
    ...
    """Parse both current-session logs once per source-file identity."""
