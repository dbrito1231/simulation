...
- **`STRUCTURE_EFFECTS_ENABLED`** (`sim_engine.py`, server-authoritative branch) — buildings do something, and duplicates stop paying. Farm plots boost edible gathers in their district (+1 per `FARM_PLOTS_PER_EXTRA` plots, capped at `FARM_YIELD_BONUS_CAP`); houses raise a population cap (`basePopulation + houses // HOUSES_PER_NEW_VILLAGER`, hard-capped at `len(AGENT_DEFS)` = 12) and `_maybe_welcome_newcomer()` moves an unused `AGENT_DEFS` villager in when there's room; workshop-station crafting requires ≥1 built Workshop **village-wide** (workshop *structures* live in village-kind districts per `PROJECT_KIND`, while craft *stations* are workshop-kind districts — don't make the check per-district) and gets +1 output once `WORKSHOPS_PER_CRAFT_BONUS` exist. On top sits **saturation** (`_type_saturated`): per-type soft caps derived from those effects; saturated types are skipped by role defaults, refused by `_start_project_for` (naming an unsaturated alternative), surfaced in the prompt (`structure_counts` + a nudge), and count as "exhausted" toward `_invention_required()` — which also stops letting a never-built crafted-goods seed (the granary) block invention while crafting is stalled (`_seed_exhausted`).
