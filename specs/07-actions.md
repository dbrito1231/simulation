...
| `accept_contract` | `target` (contract id) | `CONTRACTS_ENABLED`; contract must exist, be `open`, not be the actor's own offer, and match a directed target or `"open"` | Binds the acceptor (`status: "accepted"`). Settlement tick (`_tick_contract_settlement`, same `RULES_TICK_FRAMES` batch as mint/backstops) fulfills when the acceptor holds `qty` of `want` before `createdFrame + deadline_frames`, paying escrow to the acceptor and transferring goods to the offerer (carry-cap overflow to stockpile). Expiry refunds escrow to the offerer; an accepted-but-undelivered contract also applies a neutral→rival relationship hit from offerer to acceptor (same shape as `confront_agent`). Unaccepted open contracts expire with refund only. |
