...
| `approve_blueprint` | `target` (blueprint id), `target_district` (optional) | actor role `elder`; requires `sageReview` already `approved`/`skipped` | Registers the new structure/resource type, pops the pending blueprint, starts a district project for it (or applies as a structure upgrade if `duplicateOf` an existing type) |
...
| `change_role` | `new_role` | `new_role` must be a key in `roleRegistry`; refused if that role's `roles.json` definition has `"leader": true` (currently only `elder`) | Sets `agent["role"]` directly — the manual/LLM-chosen counterpart to `switch_role`'s auto-eligible flow. Leadership roles are excluded so only `_enact_succession_winner()` can seat an elder — [09](09-systems-society.md#succession-lifecycle_enabled-governance-slice) |
...
2. **`sage_review_blueprint`** — the Sage (elder reviewer) marks the pending
   ...
   is always required before step 3 eligibility.
3. **`approve_blueprint`** / **`reject_blueprint`** — the elder gives the final
   ...
   `approved`/`skipped`. Approval registers the new
