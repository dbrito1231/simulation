...
- **Record shapes** beyond the common `ts`/`session_id` envelope:
  ...
  - `divine`: `{type, intervention_id, request_id, frame_tick, kind,
    ...
    `restore-closed` to apply to). **Voice binding:** each recorded
    `divine_response` (valid or synthetic `missing_divine_response`) writes an
    additional `divine.jsonl` record with `status: "adherence"`, `kind`
    `divine_response`, `public: true` for public providence guidance and
    `public: false` when the ack targets a private omen (the log carries
    stance/reason/action only — never private omen text). Town-integrity kinds `repair_structures` /
    ...
    `status: "adherence"` is Voice-binding only — one record per
    `divine_response` ack (including synthetic `missing_divine_response`);
    never written for Matrix anoint/bush/story soft guidance.
