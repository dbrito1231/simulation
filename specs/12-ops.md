...
| `determinism_proof.py` | No | Emergence Breakthroughs Phase A0 — cheap determinism probe for Feature 5: two identical cold-start `SimEngine` runs with stubbed `llm_decide` (`rest`), the same `_tick_once()` count, benchmark trajectories captured via list-capturing `log_benchmark` (not live `benchmarks.jsonl`). Reports `DETERMINISM: BIT-IDENTICAL YES/NO` and, on mismatch, the first differing metric/frame/value. **Limitations:** proof/probe only — not `fork_compare.py` (A2); does not pin RNG, tick-thread scheduling, or executor ordering (A1); live LLM runs are out of scope. Uses a temp `state.db` only — never touches `simulation/state.db`. Run: `uv run python scripts/determinism_proof.py [--ticks N] [--roster N]`. |
