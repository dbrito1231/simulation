...
**Wildlife art provenance:** User-provided PNGs in `simulation/assets/wildlife/` (16 kinds: `bee`, `bird`, `boar`, `chicken`, `crab`, `deer`, `fish`, `fox`, `grazer`, `gull`, `mouse`, `owl`, `rabbit`, `seal`, `squirrel`, `turtle`). `bee` replaces the former decorative farm kind `butterfly` (same role — not huntable). Rebuild atlas: `uv run python scripts/build_wildlife_sheet.py` (keys square backdrops via border flood-fill, trims transparency; `bee.png` mint-green backdrop keyed from corner samples). Committed outputs: `wildlife.png`, source PNGs, and the build script. Preview (gitignored): `simulation/_vendor/wildlife-preview-4x.png`.
